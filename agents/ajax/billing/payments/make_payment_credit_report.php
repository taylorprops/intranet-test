...
if($id == '') {
	echo '<span id="error">ERROR - Please start over</span>';
	die();
}
...
$addInvoice = "update company.credit_report_payments set
	agent_id = '".$agent_id."',
	amount = '".$amount."',
	agent_name = '".$agent_name."',
	agent_first = '".$db -> quote($agent_first)."',
	agent_email = '".$db -> quote($email)."',
	client = '".$db -> quote($name)."',
	report_type = '".$db -> quote($report_type)."',
	charge_desc = '".$db -> quote($desc)."'
	where id = '".$id."'";
...
if($cont == 'yes') {
	...
	if($xml->isSuccessful()) {
		...
		$subject = 'Credit Report Requested by '.$agent_name;
	    $GLOB_email_top = str_replace('%%preheader%%', '', $GLOB_email_top);
	    $body = str_replace('%%subject%%', $subject, $GLOB_email_top).
		$agent_name.' just requested a credit report<br><br>
		Client: '.$name.'<br>
		Amount: $'.$amount.'<br>
		Report Type: '.$report_type.'<br><br>
		<a href="https://www.annearundelproperties.net/new/credit_reports/credit_report_requests.php">Go To Requests</a>
	    '.$GLOB_email_bottom;
		$ccs = sendTo(45);
		sendMail('', 'internal@taylorprops.com', 'internal@taylorprops.com', 'Credit Report Requested', $body, $ccs, '', '');
		//sendMail('', 'mike@taylorprops.com', 'internal@taylorprops.com', 'Credit Report Requested', $body, '', '', '');
	...
	$update = "update company.credit_report_payments set
