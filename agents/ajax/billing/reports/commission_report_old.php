...
for($i=1;$i<6;$i++) {
	$commQuery .= "recip".$i."_id = '".$id."' and (recip".$i."_date_ready >= '".$start."' and recip".$i."_date_ready <= '".$end."')";
	//$commQuery .= "(recip".$i."_id = '".$id."' and date_ready >= '".$start."' and date_ready <= '".$end."')";
	if($i < 5) {
		$commQuery .= " or ";
	}
}
...
for($c=0;$c<$commCount;$c++) {
	...
	for($i=1;$i<6;$i++) {
		...
		if($comm[$c]['recip'.$i.'_id'] == $id && $comm[$c]['recip'.$i.'_check_amount'] != '' && $comm[$c]['recip'.$i.'_check_amount'] > '0') {
			...
			if($comm[$c]['recip'.$i.'_date_ready'] != '' && $comm[$c]['recip'.$i.'_date_ready'] != '0000-00-00') {
				$commissions[$nc]['date'] = $comm[$c]['recip'.$i.'_date_ready'];
			}

			//$commissions[$nc]['date'] = $comm[$c]['date_ready'];
