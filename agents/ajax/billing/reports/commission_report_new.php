...
for($i=1;$i<6;$i++) {
	$commQuery .= "recip".$i."_id = '".$id."' and (recip".$i."_date_deposited >= '".$start."' and recip".$i."_date_deposited <= '".$end."')";
...
for($c=0;$c<$commCount;$c++) {
	...
	for($i=1;$i<6;$i++) {
		...
		if($comm[$c]['recip'.$i.'_id'] == $id && $comm[$c]['recip'.$i.'_check_amount'] != '' && $comm[$c]['recip'.$i.'_check_amount'] > '0') {
			...
			if($comm[$c]['recip'.$i.'_date_deposited'] != '' && $comm[$c]['recip'.$i.'_date_deposited'] != '0000-00-00') {
				$commissions[$nc]['date'] = $comm[$c]['recip'.$i.'_date_deposited'];
			}
...
<table style="font-size: 11px;">
	<tr>
		<th style="border-bottom: 2px solid #ccc; padding: 3px;">Date Paid</th>
