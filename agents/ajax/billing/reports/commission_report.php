	...
	for($i=1;$i<6;$i++) {
		...
		if($comm[$c]['recip'.$i.'_id'] == $id && $comm[$c]['recip'.$i.'_check_amount'] != '' && $comm[$c]['recip'.$i.'_check_amount'] > '0') {
			...
			if($comm[$c]['referral_street'] == '') {
				...
				$commissions[$nc]['StreetAddress'] = $comm[$c]['address'];

			} else {

				$commissions[$nc]['StreetAddress'] = $comm[$c]['referral_street'].' '.$comm[$c]['referral_city'].', '.$comm[$c]['referral_state'].' '.$comm[$c]['referral_zip'];
...
for($c=0;$c<count($commissions);$c++) {
		...
		<tr>
			...
			<td>'.$commissions[$c]['StreetAddress'].'</td>
		</tr>';
