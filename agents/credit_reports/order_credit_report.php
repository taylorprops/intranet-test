...
#order_another { display: none; width: 300px; margin: 150px auto; }
#order_another a { padding: 25px; font-size: 22px; }
</style>
</head>
<body>
...
<div class="body_container">
	...
	<br><br>
	<?php //if($_SESSION['S_ID'] != 3193) { echo 'This page is temporarily unavailable'; die(); } ?>
	<div class="credit-container">
		...
		<div id="upload_div">
			...
			<div class="upload_section"><strong>Select the file to upload</strong><br><br>
		...
		<div id="payment_div">
			...
			<div id="add_payment_div">
				...
				<hr>
				-->
			    <div style="padding: 15px;" id="payment_container">
					<!--
	...
	<div id="order_another">
		<a href="order_credit_report.php" class="button button_normal">Order Another Report</a>
	</div>
...
<input type="hidden" id="session_id" value="<?php echo date('YmdHis'); ?>">
...
function submit_payment(){
	...
	$.ajax({
		type: 'POST',
		data: data,
		url: '/new/agents/ajax/billing/payments/make_payment_credit_report.php',
...
function upload() {
	...
	$('#upload').uploadifive({
		...
		'queueID'          : 'queue',
		'uploadScript'     : '/new/agents/ajax/billing/payments/upload_script_credit_report.php',
		'onError'      : function(errorType) {
			alert('There was an error uploading: ' + errorType + '. Please try again');
		},
        'onUploadComplete' : function(file, data) {
			var d = $($.parseHTML(data));
			var id = d.filter('#id').text();
			if(id > 0) {
            	$('#upload_id').val(id);
				$('#payment_div').show();
			} else {
				$('#upload').uploadifive('clearQueue');
				alert('There was an error, please start over');
			}
        }
...
function reset_payment_div(){
	$('#new_card').val('');
	$('.payment_divs, #cc_options, #card_on_file_div, #payment_container').hide();
